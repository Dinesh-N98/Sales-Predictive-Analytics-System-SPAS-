...
@ApplicationScoped
public class AtRiskActivitiesService {

    @Inject
    private HotLeadsRepository hotLeadsRepository;

    public List<AtRiskActivityDto> getAtRiskActivities(int limit) {
        ...
        results.sort(Comparator.comparingDouble(this::probabilityRejected).reversed());
        ...
        List<AtRiskActivityDto> atRiskActivities = new ArrayList<>();
        for (ActivityOutcomeResult result : topResults) {
            HotLeadCandidate candidate = candidateById.get(result.getId());
            if (candidate == null) {
                continue;
            }
            Double probabilityRejected = result.getProbabilities() == null
                    ? 0.0 : result.getProbabilities().getOrDefault("Rejected", 0.0);
            atRiskActivities.add(new AtRiskActivityDto(
                    candidate.getId(),
                    candidate.getSeId(),
                    candidate.getSeName(),
        ...
        return atRiskActivities;
    }

    private double probabilityRejected(ActivityOutcomeResult result) {
        if (result == null || result.getProbabilities() == null) {
            return 0.0;
        }
        return result.getProbabilities().getOrDefault("Rejected", 0.0);
    }
}